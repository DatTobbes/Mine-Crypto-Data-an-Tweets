...
class CoinConsolidator:
    def __init__(self):
        ...
        self.DB_CONNECTION_STRING = 'mysql+pymysql://{}:{}@localhost:3306/{}?charset=utf8mb4'.format(self.DB_USER,
                                                                                                     self.DB_PASSWORD,
                                                                                                     self.DB_NAME)
    ...
    def create_tables_for_coin(self, coin_name):
        mySqlCon = MySqlDbConnector(Host=self.DB_HOST, User=self.DB_USER, Port=3306, PassW=self.DB_PASSWORD,
                                    DatabaseName=self.DB_NAME)
        mySqlCon.create_table_with_name(coin_name)

    def write_coin_to_table(self, dataframe, table_name):
